...
const Webdev: React.FunctionComponent<PageProps> = ({ data: { webdev, images } }) => {
  ...
  return (
    <Layout color={webdev.color}>
      <SEO
        ...
        individual
      />
      <Content bg={webdev.color} py={10}>
        <PBox style={imagesAnimation} px={[6, 6, 8, 10]}>
        <animated.h1 style={titleAnimation}>{webdev.title_detail}</animated.h1>
      ...
      <PBox py={10} px={[6, 6, 8, 10]}>
        <Category style={categoryAnimation}>{webdev.category}</Category>
        <Description style={descAnimation}>
          <div dangerouslySetInnerHTML={{ __html: webdev.desc }} />
        </Description>
      </PBox>
      <PBox style={{ textAlign: 'center' }} py={10} px={[6, 6, 8, 10]}>
        <h2>Want to start your own project?</h2>
        <Link to="/contactus">
        <PButton color={webdev.color} py={4} px={8}>
          Contact Us
        </PButton>
        </Link>
      </PBox>
...
export const query = graphql`
  query WebdevTemplate($slug: String!, $images: String!) {
    webdev: webdevYaml(slug: { eq: $slug }) {
      title_detail
      color
      category
      desc
